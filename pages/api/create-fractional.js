...
export default async function handler(req, res) {
  ...
  const productPayload = { 
    product: { 
      title: body.title || `Fractional: ${Date.now()}`, 
      body_html: '<strong>Fractional</strong>', 
      variants: [ 
        { price: body.price || '1.00', sku: 'FRACT-' + Date.now() } 
      ] 
    } 
  }
  ...
  const draftOrder = { 
    draft_order: { 
      line_items: [ 
        { 
          title: body.title || 'Fractional item', 
          price: body.price || '1.00', 
          quantity: 1, 
          properties: [ { name: 'fraction_quantity', value: body.quantity || 0.5 } ] 
        } 
      ], 
